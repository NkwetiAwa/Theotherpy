...
export default function UserSignUp(){
  ...
  const handleSignUp = async() => {
    ...
    if(!loading){
      setLoading(true);
      const data = { name: name, phone: phone, email: email, password: password, doctor: false }
  ...
  const loadAsync = async(token) => {
    const user = { token: token, doc: false }
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={{ marginTop: '-20%', width: '94%', padding: 10, paddingBottom: 20, borderRadius: 8, borderTopLeftRadius: 15, borderTopRightRadius: 15, backgroundColor: 'white', elevation: 5 }}>
        ...
        <View style={styles.form}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('UserSignIn')} style={{ alignItems: 'center', marginTop: 15 }}>
